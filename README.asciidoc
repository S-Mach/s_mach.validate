...
* You want to write validation code that can be used for any serialization format.
...
validation that prevents creating invalid instances) to allow mixing with serialization specific validation, fixing,
serializing or transmitting structurally correct but possibly invalid data.
* You want to leverage macros to avoid case class boiler plate when writing validation code.
